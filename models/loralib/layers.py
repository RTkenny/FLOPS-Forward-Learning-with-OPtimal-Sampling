...
class Linear(nn.Linear, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, add_noise=False):
        self.sum_log_prob = torch.zeros(x.shape[0]).to(self.log_noise_std.device)
        def T(w):
            return w.transpose(0, 1) if self.fan_in_fan_out else w

        if self.r > 0 and not self.merged:
            result = F.linear(x, T(self.weight), bias=self.bias)
            logit_output = result + (self.lora_dropout(x) @ self.lora_A.transpose(0, 1) @ self.lora_B.transpose(0, 1)) * self.scaling

        else:
            logit_output = F.linear(x, T(self.weight), bias=self.bias)
